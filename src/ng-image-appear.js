...
var ngImageAppear = angular.module('ngImageAppear', []).directive('ngImageAppear',['$timeout', function($timeout) {
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            loaderStyle = 'width: 40px; height: 40px; position: absolute; left: calc((100% - 40px) / 2); top: calc((100% - 40px) / 2);',
            defaultPlaceholderStyles = 'background-size: cover; background-repeat: no-repeat; background-position: center center; background-color: '+defaultBackgroundColor+';',
            loaderObject = {
                'style': loaderStyle
            };
            ...
            if(placeholderImgAttr != undefined) {
                if(placeholderImgAttr === '') {
                    wrapperStyles += defaultPlaceholderStyles+'background-image: url('+defaultPlaceholder+')';
                }
                else {
                    wrapperStyles += defaultPlaceholderStyles+'background-image: url('+placeholderImgAttr+')';
                }
            }
